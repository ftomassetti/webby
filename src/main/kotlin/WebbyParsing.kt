package me.tomassetti.webby.parsing

import me.tomassetti.webby.WebbyLexer
import me.tomassetti.webby.WebbyParser
import me.tomassetti.webby.WebbyParser.WebbyFileContext
import me.tomassetti.webby.ast.Point
import me.tomassetti.webby.ast.WebbyFile
...
data class AntlrParsingResult(val root : WebbyFileContext?, val errors: List<Error>) {
    fun isCorrect() = errors.isEmpty() && root != null
}

fun String.toStream(charset: Charset = Charsets.UTF_8) = ByteArrayInputStream(toByteArray(charset))

object WebbyAntlrParserFacade {
    ...
    fun parse(inputStream: InputStream) : AntlrParsingResult {
        ...
        val lexer = WebbyLexer(ANTLRInputStream(inputStream))
        lexer.removeErrorListeners()
        lexer.addErrorListener(errorListener)
        val parser = WebbyParser(CommonTokenStream(lexer))
        parser.removeErrorListeners()
        parser.addErrorListener(errorListener)
        val antlrRoot = parser.webbyFile()
        return AntlrParsingResult(antlrRoot, lexicalAndSyntaticErrors)
    }

}

data class ParsingResult(val root : WebbyFile?, val errors: List<Error>) {
    fun isCorrect() = errors.isEmpty() && root != null
}


/*object WebbyParserFacade {
